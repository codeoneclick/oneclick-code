...
{	
	float3 vLightDir_01 = normalize(mul(IN.mTangentSpace,vLightDir));
	float3 vCameraEye_01 = normalize(mul(IN.mTangentSpace,vCameraEye - IN.vWorldPosition));
	...
	float3 vLightReflect = normalize(reflect(vCameraEye_01,vNormalColor));
	float vSpecularFactor = pow(max(0.0f,dot(vLightReflect, vLightDir_01)), 128.0f);
	
	float4 vDeepColor = float4(0.05f, 0.15f, 0.20f, 1.0f);
	...
	float Fog = saturate((dist - 128.0f) / 512.0f);
    vColor = lerp(vColor,float4(1.0f,1.0f,1.0f,1.0f), Fog);
    return vColor;
}
