...
{	
	float3 vLightDirPixel = normalize(mul(IN.mTangentSpace,vLightDir));
	float3 vCameraEyePixel = normalize(mul(IN.mTangentSpace,vCameraEye - IN.vWorldPosition));
	...
	float3 vLightReflect = normalize(reflect(vCameraEyePixel,vNormalColor));
    ...
    vColor = lerp(vColor,float4(vFogColor,1.0f), fFogFactor);
    return vColor;
}
