...
VOID ServiceMainProc()
{
	...
	{
		sprintf(CServiceController::Instance()->ExeFile(),"%s",pModuleFile);
		pModuleFile[dwSize-4] = 0;
		sprintf(CServiceController::Instance()->LogFile(),"%s.log",pModuleFile);
	}
	strcpy(CServiceController::Instance()->Name(),"TEST_01");

	if(_stricmp("-i",CServiceController::Instance()->CmdLineData()) == 0 || _stricmp("-I",CServiceController::Instance()->CmdLineData()) == 0)
		Install(CServiceController::Instance()->ExeFile(), CServiceController::Instance()->Name());
	else if(_stricmp("-k",CServiceController::Instance()->CmdLineData()) == 0 || _stricmp("-K",CServiceController::Instance()->CmdLineData()) == 0)
		KillService(CServiceController::Instance()->Name());
	else if(_stricmp("-u",CServiceController::Instance()->CmdLineData()) == 0 || _stricmp("-U",CServiceController::Instance()->CmdLineData()) == 0)
		UnInstall(CServiceController::Instance()->Name());
	else if(_stricmp("-s",CServiceController::Instance()->CmdLineData()) == 0 || _stricmp("-S",CServiceController::Instance()->CmdLineData()) == 0)
		RunService(CServiceController::Instance()->Name());
	else
		ExecuteSubProcess();
}
...
{  
	...
	if (schSCManager==0) 
	{
		...
		WriteLog(CServiceController::Instance()->LogFile(), pTemp);
	}
	else
	{
...
VOID UnInstall(char* pName)
{
	...
	DeleteFile(CServiceController::Instance()->LogFile());
}
