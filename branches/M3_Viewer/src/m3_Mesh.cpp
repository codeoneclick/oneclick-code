...
void m3_Mesh::RenderBone()
{
	...
	glPointSize(2.0f);
	for(unsigned int i = 0; i < _complexMeshData->vertexDataPtr->nBones; ++i)
	{
		m3_BoneData *parentPtr = &_complexMeshData->vertexDataPtr->boneDataPtr[i];
		Matrix4x4 mBone;
		while(parentPtr != NULL)
		{
			for(uint32 seq_index = 0; seq_index < _complexMeshData->vertexDataPtr->_sequenceBlock.nTransformationIds; ++seq_index)
			{
				if(parentPtr->translationId.animid == _complexMeshData->vertexDataPtr->_sequenceBlock.transformationId[seq_index])
				{
					uint16 tId = _complexMeshData->vertexDataPtr->_sequenceBlock.transformationIndex[seq_index].tranformationDataIndex;
					uint16 tIndex = _complexMeshData->vertexDataPtr->_sequenceBlock.transformationIndex[seq_index].sequenceDataIndex;
					if(tIndex == 2)
					{
						tSpeed = _complexMeshData->vertexDataPtr->_sequenceBlock.tSequenceData[tId].nFrames/animSpeed;
						if(tFrames >= _complexMeshData->vertexDataPtr->_sequenceBlock.tSequenceData[tId].nFrames)
							tFrames = 0;
						parentPtr->_startPosition = _complexMeshData->vertexDataPtr->_sequenceBlock.tSequenceData[tId].keys[(int)tFrames];
					}
				}
			}
			...
			for(uint32 seq_index = 0; seq_index < _complexMeshData->vertexDataPtr->_sequenceBlock.nTransformationIds; ++seq_index)
			{
				if(parentPtr->rotationId.animid == _complexMeshData->vertexDataPtr->_sequenceBlock.transformationId[seq_index])
				{
					uint16 tId = _complexMeshData->vertexDataPtr->_sequenceBlock.transformationIndex[seq_index].tranformationDataIndex;
					uint16 tIndex = _complexMeshData->vertexDataPtr->_sequenceBlock.transformationIndex[seq_index].sequenceDataIndex;
					if(tIndex == 3)
					{
						rSpeed = _complexMeshData->vertexDataPtr->_sequenceBlock.rSequenceData[tId].nFrames/animSpeed;
						if(rFrames >= _complexMeshData->vertexDataPtr->_sequenceBlock.rSequenceData[tId].nFrames)
							rFrames = 0;

						parentPtr->_startRotation[0] = _complexMeshData->vertexDataPtr->_sequenceBlock.rSequenceData[tId].keys[(int)rFrames].x;
						parentPtr->_startRotation[1] = _complexMeshData->vertexDataPtr->_sequenceBlock.rSequenceData[tId].keys[(int)rFrames].y;
						parentPtr->_startRotation[2] = _complexMeshData->vertexDataPtr->_sequenceBlock.rSequenceData[tId].keys[(int)rFrames].z;
						parentPtr->_startRotation[3] = _complexMeshData->vertexDataPtr->_sequenceBlock.rSequenceData[tId].keys[(int)rFrames].w;
					}
				}
			}
	...
	for(uint32 i = 0; i < _vertexBufferPtr->nVerteces; ++i)
	{
...
void m3_Mesh::Render()
{
	...
	cgGLSetParameter1f( _cgShaderParams.cg_bLightFlag, m3_Input::enableLight);
