...
//	SetTexture(0,L"dxutcontrols.dds");
//    InitDefaultElements();
//}


//--------------------------------------------------------------------------------------
void CDXUTDialog::Init( CDXUTDialogResourceManager* pManager,LPCWSTR pszControlTextureFilename )
{
    m_pManager = pManager;
   // if( bRegisterDialog )
    pManager->RegisterDialog( this );
...
HRESULT CDXUTDialog::OnRender9( float fElapsedTime )
{
    ...
    {
        m_fTimeLastRefresh = timeGetTime();
        Refresh();
    }
...
void CDXUTDialog::InitDefaultElements()
{
    ...
    //-------------------------------------
    SetRect( &rcTexture, 256, 0, 512, 256 );
...
CDXUTEditBox::CDXUTEditBox( CDXUTDialog* pDialog )
{
    ...
    m_dfLastBlink = timeGetTime();//DXUTGetGlobalTimer()->GetAbsoluteTime();
    s_bHideCaret = false;
    m_nFirstVisible = 0;
...
void CDXUTEditBox::Render( float fElapsedTime )
{
    ...
    // Blink the caret
    //
    if( timeGetTime()/*DXUTGetGlobalTimer()->GetAbsoluteTime()*/ - m_dfLastBlink >= m_dfBlink )
    {
