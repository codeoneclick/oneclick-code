...
void gMeshContainer::_CalculateTBN(unsigned int nVerteces,unsigned int nIndeces)
{
...
void gMeshContainer::_FillTexturesList()
{
	for(int i = 0; i < this->nTextures; ++i)
	{
		this->texturesColorList.push_back(gCore::TexturesService()->Find(this->textureColorFileName[i]));
		this->texturesNormalList.push_back(gCore::TexturesService()->Find(this->textureNormalFileName[i]));
	}
}