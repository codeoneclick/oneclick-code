...
void TextureService::_insertTextureData(std::string key)
{
	...
	if(textureDataIterator == _textureDataContainer.end())
		_textureDataContainer.insert(std::pair<std::string,DDS_IMAGE_DATA*>(key,LoaderDDS::LoadImageData(key)));
}
	...
	else
	{
		...
		while(_requestIterator != _requestTexture.end())
		{
			if(value == (*_requestIterator))
			{
				...
				if(textureDataIterator != _textureDataContainer.end())
				{
					GLuint textureId =  LoaderDDS::Load(_textureDataContainer.find(value)->second);
