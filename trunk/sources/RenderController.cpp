...
void CRenderController::Render2Texture(Video::CRenderController::ERenderTexture value)
{
	switch(value)
	{
		...
		case CRenderController::REFLECTION_TEXTURE :
		{
			...
			math::vector::Vector3d vCameraPosition = cameraPtr->vPosition;
			vCameraPosition.y = -cameraPtr->vPosition.y + waterLevel * 2.0f;

			math::vector::Vector3d vCameraLookAt = cameraPtr->vLookAt;
			vCameraLookAt.y = -cameraPtr->vLookAt.y + waterLevel * 2.0f;
			cameraPtr->mView = math::matrix::MatrixView(vCameraPosition, vCameraLookAt, math::vector::Vector3d(0.0f,-1.0f,0.0f));
			...
			cameraPtr->mView = math::matrix::MatrixView(cameraPtr->vPosition,cameraPtr->vLookAt, math::vector::Vector3d(0.0f,1.0f,0.0f));
		}
		break;
