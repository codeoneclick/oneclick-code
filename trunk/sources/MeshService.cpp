...
{
	std::map<std::string,type::VertexBufferData*>::iterator meshDataIterator = _meshDataContainer.find(key);
	if(meshDataIterator == _meshDataContainer.end())
		_meshDataContainer.insert(std::pair<std::string,type::VertexBufferData*>(key,loader::Loader3DS::LoadFileData(key)));
}

void MeshService::_insertMesh(std::string key,type::VertexBufferData* value)
{
	loader::Loader3DS::WriteInVideoMemory(value);
...
type::VertexBufferData* MeshService::GetMesh(std::string value)
{
	std::map<std::string,type::VertexBufferData*>::iterator meshIterator = _meshDataContainer.find(value);
	...
	while(_requestIterator != _requestMeshContainer.end())
	{
		...
		{
			std::map<std::string,type::VertexBufferData*>::iterator meshDataIterator = _meshDataContainer.find(value);
			if(meshDataIterator != _meshDataContainer.end())
			{
