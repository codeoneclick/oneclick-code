...
void CSceneMgr::Render(void)
{
    ...
    while (pBIterator != pEIterator)
    {
        (*pBIterator)->Render(INode::E_RENDER_MODE_SIMPLE);
        ++pBIterator;
    }
    ...
    m_pRenderMgr->BeginDrawReflectionWorldSpaceScene();
    
    pBIterator = m_lContainer.begin();
    pEIterator = m_lContainer.end();
    
    while (pBIterator != pEIterator)
    {
        if((*pBIterator)->Get_ForReflection())
        {
            (*pBIterator)->Update();
            (*pBIterator)->Render(INode::E_RENDER_MODE_REFLECTION);
        }
        ++pBIterator;
    }
    m_pRenderMgr->EndDrawReflectionWorldSpaceScene();
