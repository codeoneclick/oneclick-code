const char* ShaderColorVertexDiffuseV = STRINGIFY(
...
void main(void)
{
    ...
    vec4 v_pos_world =  u_m_world * vec4(u_in_slot_vertex, 1.0);
    v_v_out_light = normalize( vec4(u_v_light,1.0) - v_pos_world).xyz;
    v_v_out_normal = (u_m_world * vec4(u_in_slot_normal, 0.0)).xyz;
}
);
