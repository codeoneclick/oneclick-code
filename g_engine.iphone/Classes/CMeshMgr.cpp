...
IResource* CMeshMgr::Load(std::string _sName, IResource::E_PARSER _eParser, IResource::E_THREAD _eThread)
{
    CMesh* pMesh = NULL;
        ...
        else
        {
            ...
            switch (_eParser) 
            {
                case IResource::E_M3_PARSER:
                    pParser = new CParser_M3(); 
                    break;
                case IResource::E_MD2_PARSER:
                    pParser = new CParser_MD2(); 
                case IResource::E_GMESH_PARSER:
                    pParser = new CParser_GMESH();
                    break;
                default:
                    break;
            }
    ...
    else if(_eThread == IResource::E_BACKGROUND_THREAD)
    {
        ...
        else
        {
            if(m_lTaskPool.find(_sName) == m_lTaskPool.end())
            {
                ...
                {
                    case IResource::E_M3_PARSER:
                        m_lTaskPool[_sName] = new CParser_M3(); 
                        break;
                    case IResource::E_MD2_PARSER:
                        m_lTaskPool[_sName] = new CParser_MD2(); 
                    case IResource::E_GMESH_PARSER:
