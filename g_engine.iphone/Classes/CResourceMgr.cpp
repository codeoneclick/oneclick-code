...
CResourceMgr::CResourceMgr()
{
    ...
    m_lMgr[IResource::E_SEQUENCE_MGR] = new CSequenceMgr();
    m_lMgr[IResource::E_MESH_MGR] = new CMeshMgr();
...
IResource* CResourceMgr::Load(const std::string& _sName,IResource::E_MGR _eMgr, IResource::E_PARSER _eParser, IResource::E_THREAD _eThread)
{
    switch (_eParser)
    {
        case IResource::E_PVR_PARSER :
            return m_lMgr[IResource::E_TEXTURE_MGR]->Load(_sName, _eParser, _eThread);
            break;
        case IResource::E_SEQ_PARSER :
            return m_lMgr[IResource::E_SEQUENCE_MGR]->Load(_sName, _eParser, _eThread);
            break;
        case IResource::E_M3_PARSER :
        case IResource::E_MD2_PARSER :
        case IResource::E_GMESH_PARSER :
            return m_lMgr[IResource::E_MESH_MGR]->Load(_sName, _eParser, _eThread);
            break;
        default:
            return NULL;
            break;
...
void CResourceMgr::Unload(IResource *_pResource)
{
    switch (_pResource->Get_Parser())
    {
        ...
        case IResource::E_SEQ_PARSER :
            ...
            break;
        case IResource::E_M3_PARSER :
            m_lMgr[IResource::E_MESH_MGR]->Unload(_pResource->Get_Name());
            break;
        default:
            break;
    }
