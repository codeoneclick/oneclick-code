...
void CParticleEmitter::Load(IResource::SResource _tResource)
{
    ...
    for(unsigned int index = 0; index < m_iNumParticles; index++)
    {
        ...
        pTexCoordData[index * 4 + 0] = glm::vec2(-1.0f, -1.0f);
        pTexCoordData[index * 4 + 1] = glm::vec2( 1.0f, -1.0f);
        pTexCoordData[index * 4 + 2] = glm::vec2( 1.0f,  1.0f);
        pTexCoordData[index * 4 + 3] = glm::vec2(-1.0f,  1.0f);
    ...
    for(unsigned short i = 0; i < m_iNumParticles; i++)
    {
        ...
        float fOffset = _Get_RandomFromRange(0.0f, 2.0f);
        vPosition.x = fOffset;
        fOffset = _Get_RandomFromRange(0.0f, 2.0f);
        vPosition.z = fOffset;
...
void CParticleEmitter::Update(void)
{
    ...
    for(unsigned short i = 0; i < m_iNumParticles; i++)
    {
        ...
        vPosition.y += fUpper / 50.0f;
        m_pParticles[i].m_vPosition = vPosition;
        if(m_pParticles[i].m_vPosition.y >= 2.0f)
        {
