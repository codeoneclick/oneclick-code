...
void m3_MeshService::_insertMeshData(std::string key)
{
	...
	if(meshDataIterator == _meshDataContainer.end())
	{
		switch(_checkFileExtension(key))
		{
			...
			case MESH_3DS:
				_meshDataContainer.insert(std::pair<std::string,m3_VertexBufferPtr*>(key,m3_3DSLoader::LoadFileData(key)));
			break;
		}
	}
}
			...
			if(value == (*_requestIterator))
			{
				...
				if(meshDataIterator != _meshDataContainer.end())
				{
					...
					switch(_checkFileExtension(value))
					{
						...
						case MESH_3DS:
							meshVertexBufferId = m3_3DSLoader::Load(_meshDataContainer.find(value)->second);
						break;
					}
